...
async function autoSeedProducts() {
    try {
        ...
        if (count === 0) {
            console.log('CSDL trống, đang tự động tạo 50 sản phẩm mẫu...');
            const categories = [
            ...
            for (let i = 1; i <= 50; i++) {
                ...
                newProducts.push({
                    ...
                    images: [image], image: image, stock: Math.floor(Math.random() * 10) + 1,
            ...
            console.log('Đã tự động thêm 50 sản phẩm mẫu vào CSDL!');
        }
    } catch (err) {
