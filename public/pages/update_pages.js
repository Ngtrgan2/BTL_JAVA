...
const contents = {
    ...
    'corporate-gifts.html': {
        ...
        'content': `<h2 style="color: var(--gold-primary); text-align: left; margin-bottom: 1rem;">1. Quà tặng đẳng cấp</h2>
            <p style="text-align: left; line-height: 1.8;">Trang sức kim cương và các vật phẩm chế tác độc quyền từ An LUXURY là lựa chọn hoàn hảo để triâm đối tác, khách hàng VIP, hoặc khen thưởng nhân sự xuất sắc. Món quà không chỉ mang giá trị vật chất mà còn thể hiện đẳng cấp và sự tôn trọng của doanh nghiệp.</p>
...
for (const [filename, data] of Object.entries(contents)) {
    ...
    if (fs.existsSync(filepath)) {
        ...
        // Find <div class="policy-content reveal"> ... </div>
        // And replace it
        // The regex looks for <div class="policy-content[^>]*> and the corresponding closing div before </main>
        const pattern = /(<div class="policy-content[^>]*>).*?(<\/div>\s*<\/main>)/s;
        ...
        if (newContent !== content) {
            ...
            console.log(`Failed to update ${filename} - pattern not found`);
        }
    } else {
